...
export async function getUserProfile(userId: string) {
  ...
  // Compute pending mandatory enrollments — used by the OnboardingGate to know
  // whether to block platform access. We only consider published mandatory
  // courses whose `applicableRoles` includes this user's role and which are
  // either:
  //   - not yet enrolled at the current course version, OR
  //   - enrolled but not completed and not declined
  //   - completed but expired
  // This call has a side effect: it lazily creates new enrollment rows when an
  // expired/out-of-date completion is found, and blanks the user's
  // onboardingCompletedAt flag in those cases.
  const pendingMandatoryEnrollments = user.onboardingRequired
    ? await getPendingMandatoryEnrollments(user.id, user.role)
    : [];

  const { passwordHash: _, ...userWithoutPassword } = user;

  // If we just lazily flagged this user as non-compliant, the user object we
  // fetched moments ago has a stale `onboardingCompletedAt`. Reflect the
  // current truth in the response so the frontend never sees a "completed but
  // pending" state. Source of truth for the gate is the pending list.
  if (pendingMandatoryEnrollments.length > 0) {
    (userWithoutPassword as any).onboardingCompletedAt = null;
  }

  return { user: await withAvatarUrl(userWithoutPassword), permissions, pendingMandatoryEnrollments };
}
...
async function getPendingMandatoryEnrollments(userId: string, role: string): Promise<PendingEnrollment[]> {
  const courses = await prisma.course.findMany({
    where: {
      status: 'PUBLISHED',
      isMandatoryOnHire: true,
      applicableRoles: { has: role as any },
    },
    select: { id: true, slug: true, title: true, version: true },
  });

  const now = new Date();
  const result: PendingEnrollment[] = [];
  for (const course of courses) {
    // Look up the user's most recent enrollment for this course.
    ...
    // Pending if any of:
    //  - no enrollment yet
    //  - latest is for an older course version
    //  - latest is unfinished and not declined
    //  - latest is completed but expired (annual re-ack)
    // A declined enrollment at the current version means the user permanently
    // refused — we don't keep prompting them; HR follows up out-of-band.
    const needsEnrollment = !latest || latest.courseVersion < course.version;
    const inProgress = latest && latest.courseVersion === course.version && !latest.completedAt && !latest.declinedAt;
    const expired = latest && !!latest.completedAt && latest.expiresAt !== null && latest.expiresAt <= now;

    if (needsEnrollment || inProgress || expired) {
      // Decide whether we need to lazily create a new enrollment row at the
      // current course version. Three triggering conditions:
      //   (a) user has never been enrolled in this course (!latest)
      //   (b) latest is at an older version — regardless of its state.
      //       The course got bumped past whatever they completed, declined,
      //       or left in progress. Old rows stay as immutable evidence; we
      //       always need a fresh in-progress row at the current version
      //       for the OnboardingGate to walk them through.
      //       (Bug fixed in Phase 2.2b: previously this also required
      //        `!!latest.completedAt`, which meant a user who DECLINED v1
      //        or was IN-PROGRESS on v1 when admin bumped to v2 ended up
      //        being prompted with the stale v1 enrollmentId but a
      //        courseVersion=v2 claim. Mismatch → frontend renders bogus.)
      //   (c) latest is completed but expired (annual re-ack window elapsed)
      const needsLazyCreate =
        !latest ||
        expired ||
        (latest && latest.courseVersion < course.version);

      let enrollmentId = latest?.id ?? '';
      if (needsLazyCreate) {
        // Use the latest enrollment AT THE CURRENT version if it already
        // exists and is in progress (covers concurrent /auth/me races and
        // post-version-bump returning users).
        const sameVersionLatest = await prisma.enrollment.findFirst({
          where: { userId, courseId: course.id, courseVersion: course.version },
          orderBy: { cycle: 'desc' },
        });
        if (sameVersionLatest && !sameVersionLatest.completedAt && !sameVersionLatest.declinedAt) {
          enrollmentId = sameVersionLatest.id;
        } else {
          const nextCycle = (sameVersionLatest?.cycle ?? 0) + 1;
          const fresh = await prisma.enrollment.create({
            data: { userId, courseId: course.id, courseVersion: course.version, cycle: nextCycle },
          });
          enrollmentId = fresh.id;
          // Blank the convenience flag — they're not currently compliant.
