...
/**
 * Type guard exposing the input shape needed by the knowledge-pack
 * builder. The KP returns metadata-only — no URLs, no content — and
 * the agent fetches via getDocumentDownloadUrl on demand.
 */
export type DocumentSummaryForAgent = {
  id: string;
  title: string;
  description: string | null;
  category: DocCategory;
...
/**
 * Used by agentKnowledgePack.service. Same authz model as the human
 * list — agent must be a project member. Returns the same shape minus
 * uploadedBy (the agent doesn't need to know who uploaded what for the
 * KP listing; it can fetch the doc and look at the audit metadata if
 * it ever needs to).
 */
export async function listDocumentsForKnowledgePack(
  projectId: string,
  agentUserId: string,
): Promise<DocumentSummaryForAgent[]> {
  await assertProjectMember(projectId, agentUserId);
