...
/**
 * Fire when a user submits their weekly timesheet for approval.
 *
 * Recipients: every user with `analytics.view_team` permission (i.e.
 * the org's ADMIN + PM tier — the approval audience). Without this
 * ping the approval queue was a silent fan-in — approvers had to
 * manually check `/timesheet/pending` to see new submissions.
 *
 * The submitter is excluded from the recipient set (they obviously
 * know they submitted).
 */
export async function notifyTimesheetSubmitted(args: {
  timesheetId: string;
  submittedBy: string;
  ...
  const approvers = await prisma.user.findMany({
    where: {
      role: { in: ['ADMIN', 'PRODUCT_MANAGER', 'SUPER_ADMIN'] },
      isActive: true,
      id: { not: args.submittedBy },
    },
    select: { id: true },
  });
  ...
  await createBulkNotifications(
    approvers.map((a) => ({
      ...
      body: `${args.submittedByName} submitted ${args.totalHours.toFixed(1)} hours for week of ${args.weekStart}`,
      link: '/admin/timesheet-approvals',
    })),
  );
}

/**
 * Fire when a submitter's timesheet is approved. Single recipient
 * (the submitter); body inlines the approver's name so the
 * notification is actionable without opening it.
 */
export async function notifyTimesheetApproved(args: {
  ...
  await createNotification({
    ...
    body: `${args.approverName} approved your ${args.totalHours.toFixed(1)} hours for week of ${args.weekStart}`,
    link: '/eng/timesheet',
  });
}

/**
 * Fire when a submitter's timesheet is rejected. The reason text is
 * the most actionable piece of info — surface it in the body so the
 * recipient knows what to fix without opening the timesheet page.
 *
 * Pre-2026-05-15 the rejection reason was captured on TimesheetWeek
 * but NEVER surfaced — engineers saw their submission disappear
 * from "pending" with no signal it had been rejected (let alone
 * why). This notification closes that loop.
 */
export async function notifyTimesheetRejected(args: {
  submitterUserId: string;
  approverName: string;
  weekStart: string;
  reason: string;
  ...
  await createNotification({
    ...
    body: `${args.approverName} rejected your week of ${args.weekStart}: ${args.reason}`,
    link: '/eng/timesheet',
  });
}
...
/**
 * Fire when an external website submits a new lead via the public
 * ingest endpoint. Recipients = every active user whose role grants
 * `leads.view`. The lead row was just created, so the link drops the
 * recipient straight into the lead detail page.
 *
 * Duplicates (same email + formType within 7 days) reuse the existing
 * row and SHOULD NOT re-notify — the caller in lead.service skips
 * this helper on the duplicate branch.
 */
export async function notifyLeadIngested(args: {
  leadId: string;
  projectId: string;
  projectName: string;
  formType: string;
  ...
  // No per-user permission grants exist in this codebase — role
  // membership is the full picture. Resolve roles that grant
  // leads.view, then fan out to active users in those roles.
  const grants = await prisma.rolePermission.findMany({
    where: { granted: true, permission: { key: 'leads.view' } },
    select: { role: true },
  });
  ...
  const who = args.leadName?.trim() || args.leadEmail || 'Someone';
  const body = `${who} submitted the ${args.formType} form on ${args.projectName}.`;
